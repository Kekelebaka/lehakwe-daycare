...
r.get('/messages/threads', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT c.child_id, c.full_name AS child_name, c.parent_id,
     ...
     FROM children c
     LEFT JOIN parents p ON c.parent_id = p.parent_id AND p.centre_id = c.centre_id
...
r.get('/messages/thread/:childId', async (c) => {
  ...
  const child = await c.env.DB.prepare('SELECT child_id, full_name, parent_id FROM children WHERE child_id = ? AND centre_id = ?').bind(childId, centre).first<any>();
  const parent = child?.parent_id
    ? await c.env.DB.prepare('SELECT parent_id, full_name, email FROM parents WHERE parent_id = ? AND centre_id = ?').bind(child.parent_id, centre).first<any>()
    : null;

  await markThreadRead(c.env.DB, t.thread_id, 'staff', centre);
  const msgs = await c.env.DB.prepare(
    'SELECT message_id, sender_type, sender_id, sender_name, body, read_at, created_at FROM thread_messages WHERE thread_id = ? AND centre_id = ? ORDER BY created_at ASC',
...
r.post('/messages/thread/:childId', async (c) => {
  ...
  const msg = await insertThreadMessage(c.env.DB, t.thread_id, 'staff', identity.sub, identity.name || 'Staff', parsed.data.body, centre);
