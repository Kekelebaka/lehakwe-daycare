...
r.get('/payslips', async (c) => {
  ...
  let q = 'SELECT * FROM payslips';
  const p: any[] = [];
  if (month && year) {
    q += ' WHERE pay_period_month = ? AND pay_period_year = ?';
...
r.post('/payslips', async (c) => {
  ...
  ).bind(id, d.staff_id, d.pay_period_month, d.pay_period_year, d.gross_pay, d.total_deductions, d.net_pay).run();
...
r.post('/payslips/:id/email', async (c) => {
  ...
  const db = initDb(c.env.DB);
  await c.env.DB.prepare("UPDATE payslips SET status = 'emailed' WHERE payslip_id = ?").bind(id).run();
...
r.post('/payslips/:id/paid', async (c) => {
  const id = c.req.param('id');
  const db = initDb(c.env.DB);
  await c.env.DB.prepare("UPDATE payslips SET status = 'paid' WHERE payslip_id = ?").bind(id).run();
...
r.get('/fees/schedules', async (c) => {
  const rows = await c.env.DB.prepare('SELECT * FROM fee_schedules WHERE active = 1 ORDER BY monthly_fee ASC').all();
  return c.json({ ok: true, data: rows.results });
});
...
r.post('/fees/schedules', async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO fee_schedules (schedule_id, age_group, monthly_fee, description) VALUES (?, ?, ?, ?)').bind(id, d.age_group, d.monthly_fee, d.description || null).run();
...
r.post('/fees/records', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO fee_records (fee_id, child_id, schedule_id, month, year, amount_due, amount_paid, payment_method, payment_date, status, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).bind(id, d.child_id, d.schedule_id || null, d.month, d.year, d.amount_due, d.amount_paid || 0, d.payment_method || null, d.payment_date || null, d.status || 'pending', d.notes || null).run();
  return c.json({ ok: true, data: { id } });
});
...
r.put('/fees/records/:id', async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE fee_records SET ${fields.join(', ')} WHERE fee_id = ?`).bind(...vals).run();
