...
r.get('/me', async (c) => {
  ...
  const db = initDb(c.env.DB);
  const staffRow = await db.DB.prepare('SELECT * FROM staff WHERE staff_id = ? AND active = 1 LIMIT 1').bind(identity.sub).first<any>();
  if (!staffRow) return c.json({ ok: false, error: 'Staff record not found' }, 404);
  return c.json({ ok: true, data: { id: staffRow.staff_id, name: staffRow.full_name, email: staffRow.email, role: identity.role, signature: staffRow.signature || '', active: staffRow.active } });
});
