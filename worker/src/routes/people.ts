...
r.get('/children', async (c) => {
  const rows = await c.env.DB.prepare('SELECT * FROM children ORDER BY full_name ASC').all();
...
r.post('/children', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO children (child_id, full_name, date_of_birth, age_group, enrolment_date, status, parent_id, emergency_contact_name, emergency_contact_phone, medical_notes, allergies, pickup_authorisation_notes, gender, race, disability, disability_description, income_category, id_number, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
  ).bind(id, d.full_name, d.date_of_birth || null, d.age_group || null, d.enrolment_date || null, d.status || 'active', d.parent_id || null, d.emergency_contact_name || null, d.emergency_contact_phone || null, d.medical_notes || null, d.allergies || null, d.pickup_authorisation_notes || null, d.gender || null, d.race || null, d.disability || null, d.disability_description || null, d.income_category || null, d.id_number || null).run();
...
r.put('/children/:id', async (c) => {
  ...
  ).bind(d.full_name ?? null, d.date_of_birth ?? null, d.age_group ?? null, d.enrolment_date ?? null, d.status ?? null, d.parent_id ?? null, d.emergency_contact_name ?? null, d.emergency_contact_phone ?? null, d.medical_notes ?? null, d.allergies ?? null, d.pickup_authorisation_notes ?? null, d.gender ?? null, d.race ?? null, d.disability ?? null, d.disability_description ?? null, d.income_category ?? null, d.id_number ?? null, id).run();
...
r.delete('/children/:id', async (c) => {
  const id = c.req.param('id');
  const db = initDb(c.env.DB);
  await c.env.DB.prepare('DELETE FROM children WHERE child_id = ?').bind(id).run();
...
r.get('/parents', async (c) => {
  const rows = await c.env.DB.prepare('SELECT * FROM parents ORDER BY full_name ASC').all();
  return c.json({ ok: true, data: rows.results });
});
...
r.post('/parents', async (c) => {
  ...
  const id = db.uuid();
  await c.env.DB.prepare(
    `INSERT INTO parents (parent_id, full_name, phone, email, address, relationship_to_child, emergency_contact, notes, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
  ).bind(id, d.full_name, d.phone || null, d.email || null, d.address || null, d.relationship_to_child || null, d.emergency_contact || 0, d.notes || null).run();
...
r.put('/parents/:id', async (c) => {
  ...
  const db = initDb(c.env.DB);
  await c.env.DB.prepare(
    `UPDATE parents SET full_name = COALESCE(?, full_name), phone = COALESCE(?, phone), email = COALESCE(?, email), address = COALESCE(?, address), relationship_to_child = COALESCE(?, relationship_to_child), emergency_contact = COALESCE(?, emergency_contact), notes = COALESCE(?, notes), updated_at = datetime('now') WHERE parent_id = ?`,
  ).bind(d.full_name ?? null, d.phone ?? null, d.email ?? null, d.address ?? null, d.relationship_to_child ?? null, d.emergency_contact ?? null, d.notes ?? null, id).run();
...
r.delete('/parents/:id', async (c) => {
  const id = c.req.param('id');
  const db = initDb(c.env.DB);
  await c.env.DB.prepare('DELETE FROM parents WHERE parent_id = ?').bind(id).run();
...
r.post('/staff', async (c) => {
  ...
  const id = db.uuid();
  await c.env.DB.prepare(
    `INSERT INTO staff (staff_id, full_name, id_number, employee_number, job_title, email, phone, start_date, basic_salary, uif_enabled, paye_enabled, active, signature, emergency_contact_name, emergency_contact_phone, notes, gender, race, disability, disability_description, training_received, training_type, subsidised, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
  ).bind(id, d.full_name, d.id_number || null, d.employee_number || null, d.job_title, d.email || null, d.phone || null, d.start_date || null, d.basic_salary || 0, d.uif_enabled ?? 1, d.paye_enabled ?? 0, d.active ?? 1, d.signature || '', d.emergency_contact_name || null, d.emergency_contact_phone || null, d.notes || null, d.gender || null, d.race || null, d.disability || null, d.disability_description || null, d.training_received || null, d.training_type || null, d.subsidised ?? 1).run();
...
r.put('/staff/:id', async (c) => {
  ...
  ).bind(d.full_name ?? null, d.id_number ?? null, d.employee_number ?? null, d.job_title ?? null, d.email ?? null, d.phone ?? null, d.start_date ?? null, d.basic_salary ?? null, d.uif_enabled ?? null, d.paye_enabled ?? null, d.emergency_contact_name ?? null, d.emergency_contact_phone ?? null, d.notes ?? null, d.gender ?? null, d.race ?? null, d.disability ?? null, d.disability_description ?? null, d.training_received ?? null, d.training_type ?? null, d.subsidised ?? null, id).run();
...
r.delete('/staff/:id', async (c) => {
  const id = c.req.param('id');
  const db = initDb(c.env.DB);
  await c.env.DB.prepare("UPDATE staff SET active = 0, updated_at = datetime('now') WHERE staff_id = ?").bind(id).run();
